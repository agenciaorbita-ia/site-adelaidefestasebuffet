...
1. Ler `_memoria/empresa.md` para extrair: produtos/serviços, região, público-alvo, diferenciais
...
3. Usar **WebSearch** para cada grupo de termos:
   - Buscar `"[termo] site:trends.google.com"` para verificar sazonalidade
   - Buscar `"[termo]"` para observar o que aparece (orgânico, ads, maps)
   - Buscar `"[termo] related searches"` para expandir a lista
...
1. Selecionar os **top 10 termos** do Passo 1
2. Para cada termo, usar **WebSearch** e analisar:
...
3. Para cada concorrente relevante (máximo de 5 a 8), usar **WebFetch** para analisar:
...
4. Identificar:
   ...
   - **Ameaças:** concorrentes fortes demais para enfrentar diretamente
...
**Objetivo:** Montar o perfil completo do Google Business Profile para aparecer no Maps e no Local Pack.
...
2. Criar documento com **tudo que precisa ser preenchido/otimizado:**
   ...
   **Informações básicas:**
   ...
   - Categoria principal + secundárias (sugerir as mais adequadas ao nicho)
   ...
   **Citações e diretórios:**
   - Lista de diretórios relevantes para o nicho
   - NAP consistente (Name, Address, Phone) em todas as listagens
...
**Objetivo:** Garantir que cada página esteja otimizada para as palavras-chave certas.

1. Ler a estrutura atual do site (se `site/` existir; caso contrário, perguntar quais são as páginas)
2. Para cada página:
   ...
   **Schema Markup (dados estruturados):**
   ...
   - Product schema para os produtos
...
- Schema markup pronto para copiar (JSON-LD)
...
1. Baseado nos termos do Passo 1 (especialmente os informacionais):
   ...
   **Páginas/posts evergreen:**
   ...
   - Para cada ideia: título otimizado, keyword-alvo, estrutura de headings, estimativa de tamanho

   **Cluster de conteúdo:**
   - Página pilar
   - Páginas satélite que apontam para a página pilar
...
### Passo 6 — GOOGLE ADS: Campanhas prontas para executar
...
2. **Estrutura de campanhas:**
   ...
   **Search:**
   ...
   - Para cada grupo: 10-15 palavras-chave, lista de negativas, 3 RSAs, extensões
   - Orçamento diário, estratégia de lance, segmentação geográfica

   **Local (se aplicável):** anúncios para Google Maps, segmentação por proximidade
...
> A skill `/anuncio-google` consome esse arquivo e gera o CSV pronto para importar no Google Ads.
...
**Objetivo:** Otimizar a presença para que IAs generativas (ChatGPT, Gemini, Perplexity, Copilot) citem a empresa quando alguém perguntar sobre o nicho.

**Por que importa:** Um número crescente de clientes pergunta a IAs "qual o melhor fornecedor/serviço de X em Y?" — quem aparece capta um lead qualificado sem investir em anúncios pagos.

1. **Auditoria GEO:**
   - WebSearch nos top 10 termos em engines de IA (Perplexity, etc.)
   - Verificar se a empresa (ou concorrentes) aparece
   - Mapear quais fontes as IAs citam para esse nicho

2. **Conteúdo otimizado para IA:**
   ...
   - Evitar texto vago — conteúdo genérico tende a ser descartado pelas IAs
...
**Output:** `marketing/seo/08-geo-otimizacao-ia.md` com auditoria, FAQ + schema JSON-LD, lista de ações para aumentar citações, checklist de monitoramento.
...
- Focar em termos com intenção comercial/transacional para negócios B2C/B2B locais
