...
description: >
  ...
  Recebe um tema (manual ou da estratégia de conteúdo do SEO), escreve o artigo de blog completo,
  gera o carrossel resumo via skill /carrossel, e produz as legendas para Instagram, Facebook e
  LinkedIn — tudo interligado, com o carrossel direcionando para o blog.
...
Skill orquestradora. Recebe um tema → entrega artigo no blog + carrossel + 3 legendas (Insta, FB, LinkedIn), tudo conectado.
...
- **Skill carrossel:** `.claude/skills/carrossel/SKILL.md` — usar para a etapa do carrossel
...
Se nenhum tema for informado → ler `marketing/seo/05-estrategia-conteudo.md`, listar os artigos satélite + a página pilar, e perguntar:

> "Qual tema da estratégia? (lista de opções)"

Identificar quais temas já foram publicados como blog (verificando a pasta correspondente) para evitar duplicação.
...
- Como os concorrentes abordam o tema (`02-analise-concorrencia.md`) — para evitar repetir o óbvio
...
**Sempre começar com `draft: true`.** O usuário revisa e altera para `false` quando aprovar.
...
- Markdown limpo: `##` para H2, `###` para H3, listas com `-`, links em `[texto](url)`

### Passo 3 — Carrossel resumo

**Sem necessidade de confirmação prévia, iniciar diretamente a criação do carrossel** chamando `.claude/skills/carrossel/SKILL.md` (tipo 1: carrossel texto puro).
...
- **Slides 2-6:** os pontos-chave do blog (1 ideia por slide, em frase natural, evitando tópicos secos)
- **Slide final — CTA para o blog:** "Texto completo no nosso blog" + URL `<dominio>/blog/<slug>`
...
- CTA para o carrossel ("Arraste pro lado") + CTA para o blog ("Texto completo no link da bio" ou URL direta)
...
Ao final, apresentar ao usuário uma lista clara:
...
Para publicar:
1. Revisar o blog e alterar para draft: false
2. Rebuild do site (se Astro/Hugo/etc) ou copiar para o CMS
3. Renderizar PNGs do carrossel: cd marketing/conteudo/<pasta> && node render.js
4. Publicar o carrossel no Instagram e Facebook com legenda.md (ou usar /aprovar-post)
5. Publicar texto e link no LinkedIn com legenda-linkedin.md
```
...
- Atualização de artigo existente → editar diretamente o arquivo .md
- Post único, frase de impacto → `/carrossel`

## Princípios

1. **Blog é a peça-mãe.** Carrossel e legendas são derivados dele, não o contrário.
2. **Tudo interligado.** Cada peça referencia a outra (o carrossel remete ao blog, o blog contém CTA para contato).
