...
description: >
  Escreve respostas curtas e humanas para as avaliações do Google Meu Negócio.
  Mantém o padrão: nome do cliente, agradecimento variado, frase concreta sobre
  o produto/serviço da empresa, emoji opcional. Sem respostas automáticas e
  genéricas, características de grandes empresas.
...
Respostas são **curtas** (1 a 2 frases), **pessoais** e **concretas**. Sem respostas automáticas e genéricas, características de grandes empresas.
...
2. **Sempre agradecer.** Variar: "Obrigado", "Muito obrigado", "Que bom", "Valeu" — para não parecer robótico.
3. **Frase concreta, não genérica.** Referenciar algo específico da review ou algo da empresa (produto, processo, cuidado, tradição). Evitar "seu feedback é muito importante pra nós", "estamos sempre à disposição", "agradecemos a preferência".
4. **Emoji no final — opcional, na maioria das vezes sim.** Usar em reviews calorosas/elogiosas. Evitar em reviews formais, curtas e secas, ou em críticas. Nunca mais de 1 emoji.
...
O usuário vai colar texto, print ou lista de reviews. Extrair para cada uma:
- Nome do autor
- Nota (estrelas)
...
Para cada review, escrever **uma única resposta** seguindo o padrão acima.
...
| 3★ ou menos | **Parar e alinhar com o usuário antes de responder** — review negativa merece resposta sob medida. Não responder de forma automática. |

### Passo 3 — Entregar em formato fácil de copiar

Listar cada resposta abaixo da review, pronta para colar no GMB. Formato:
...
Se o usuário pedir para guardar, salvar em `marketing/avaliacoes-google/YYYY-MM-DD-respostas.md` com review + resposta publicada. Só criar a pasta se o usuário quiser registro. Não fazer por padrão.

---

## Regras

- Nunca copiar respostas idênticas para reviews diferentes. Variar sempre.
