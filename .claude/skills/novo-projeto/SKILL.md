...
description: >
  ...
  "novo cliente", "/novo-projeto", "começar projeto pra X" ou pedir para estruturar um trabalho novo.
---
...
4. "Que tipo de entrega está prevista? (ex: ads, site, conteúdo, automação, proposta — pode ser mais de uma)"

### Passo 2 — Decidir local

Baseado na resposta 2:

- **Cliente novo:** criar em `clientes/<Nome>/` (ou na pasta equivalente do perfil — ler `CLAUDE.md` da raiz para confirmar a convenção)
...
[Vazio — preencher com regras que valem apenas para este projeto, à medida que forem identificadas]
```
...
Quando for trabalhar nesse projeto, abra o terminal já dentro da pasta — assim o CLAUDE.md específico é carregado automaticamente junto com o da raiz.
```

## Regras

- Nome de pasta: usar o nome conforme informado pelo usuário, sem normalizar agressivamente (manter acentos; espaços convertidos em hífen, preservando um nome reconhecível)
- Não criar subpastas que não foram solicitadas (por exemplo, "para organizar melhor"). Criar apenas o que foi mencionado nas entregas
- Se o cliente/projeto já existe (pasta com mesmo nome), avisar e perguntar se é para adicionar dentro ou criar com sufixo
