...
name: carrossel
description: >
  Cria carrosséis e posts visuais para Instagram, TikTok, LinkedIn com a identidade visual da marca.
...
Skill central de criação de conteúdo visual: a partir de um tema, entrega HTMLs estilizados, PNGs prontos para publicação e legenda no padrão da marca.
...
- **Playwright:** para renderizar HTML em PNG (`npx playwright screenshot` ou via `render.js`)
- **OpenAI API (opcional):** para gerar fotos realistas — apenas se o cliente tiver a chave configurada
...
O ÓrbitaOS tem um estilo próprio — editorial, calmo, premium. Sem clip-art, sem emoji decorativo, sem gradiente arco-íris, sem template genérico de IA. `identidade/design-guide.md` sobrescreve esses padrões; quando o design-guide for vago ou estiver em branco, usar o que está descrito aqui (não interromper o fluxo para solicitar `/instalar` — o `/carrossel` funciona com valores padrão adequados).
...
Paleta sóbria: fundo dark + off-white + **UMA** cor de destaque. Nunca quatro cores competindo entre si.
...
- **Stamps circulares** (200x200, border 3px translúcida, rotate -10deg) para selos/datas/dados
- **Tags/pills** uppercase, padding generoso, kerning 0.2em, para rotular a categoria do slide
- Padding base: 70-100px nas laterais

### Layouts nomeados

Vocabulário de layout — cada slide tem um nome. Variar entre eles para criar ritmo:
...
Antes de definir a capa, considerar a **última capa publicada** para alternar:
...
Seguir `_memoria/preferencias.md`. Em geral: frases naturais, sem jargão de marketing, sem corporativês. O público real raramente utiliza termos como "ticket médio", "performance" ou "B2B" — a comunicação deve refletir a linguagem desse público.

### Legenda — sempre gerar junto

Ao terminar de renderizar os PNGs, gerar **automaticamente** a legenda do post e salvar em `legenda.md` na mesma pasta. **Não é necessário aguardar solicitação do usuário.** Estrutura padrão:

1. Hook (pergunta ou afirmação)
2. Contexto (1-2 frases sobre o conteúdo)
3. CTA para deslizar ("Arraste para o lado e confira")
...
2. Ler `identidade/design-guide.md` para cores, fontes e logo
...
Aplica-se apenas quando o usuário tiver solicitado carrossel com foto IA.
...
Se o script ainda não existir, instruir o usuário a configurar `OPENAI_API_KEY` no `.env` e criar o script (ou usar outra ferramenta de geração de imagem).

4. Apresentar a foto ao usuário antes de continuar.
...
2. Criar `render.js` na mesma pasta — script Node com Playwright que abre o HTML e tira screenshot de cada `.slide` em 1080x1350. É possível reutilizar `node_modules` de uma pasta anterior (não é necessário executar `npm install` a cada vez):
```bash
NODE_PATH="<pasta-com-node_modules>/node_modules" node render.js
```

3. Apresentar os slides 1, 2 e o CTA final renderizados. Se aprovado, apresentar os intermediários.
...
> "Este conteúdo também pode se tornar um artigo no blog. Deseja que eu crie a versão para SEO?"
...
- Carrossel: 1080x1350 (4:5 retrato) — sempre. TikTok/Reels: 1080x1920 (9:16) — apenas quando solicitado explicitamente
...
- Render: reutilizar `node_modules` quando possível (não executar `npm install` em cada pasta)
